...
code2rmd= function(.file= get.theFile(), s= readLines(.file, warn= F), toTempDir=T, show=T, verb=F) { catf('\ncode2rmd("%s"): \n', .file)  #, withLineNum=T
		stopifnot(require(knitr))
		stopifnot(require(markdown))
...
#	aa[155:160,]
	...
	kn.htm2rwj= treat.knit.html= function(theFile, rFile=theFile, dout=dirname(rFile), verb=F) { catf('\n:\n')
...
#			s1= ifelse(depth!= 0, gsub('([^#\\{\\}]*)\\}', '\\1<b>}</b></div class="D-fold">', s1), s1)
		...
		figs= gre2('class="j?fig"',, s1) ;  if(verb) prr(figs)
		figs= sub('.*(<img id=".*?>).*','\\1', figs) ;  if(verb) prr(figs)
		...
		wl(out, fp(dout, basename(rFile) %+% '.htm'), show=F)
...
rwj= RWorkJournal= r2rmd2rwj= function(theFile= get.theFile(), base64=F, verb=F, ...) { catf('\nr2rmd2rwj: %s\n', theFile)
...
ReleaseOut= function(vers='b', exec= F, theFile= get.theFile(), d= dirname(theFile)) {
#  ReleaseOut= function(theFile, vers='', exec=F) {
    #stopifnot(basename(gw()) ==  "out" && dirname(gw()) == dirname(theFile))
    
	catn(theFile, fp(d, 'out'), file.exists(theFile), file.exists(fp(d, 'out')))
	...
	if(exec){
		...
		file.copy(theFile %+% '.htm', outRelDir)
#		stopifnot(file.copy('../img', outRelDir, recursive =T))
#		stopifnot(file.copy('../out', outRelDir, recursive =T))
		
#		stopifnot(file.copy('../*', outRelDir, recursive =F))
#		exec(sf('copy ../*.* %s', outRelDir))
#		
#		stopifnot(file.rename('../img', sf('%s/img', outRelDir)))
#		#try(file.rename(sf('%s.htm', theFile), outFile), silent=TRUE)
#		
#		gw()
#		prr(adir(patt='', pattNeg='^$', path ='..', recursive = FALSE, include.dirs = FALSE))
#		prr(dir( path ='..', recursive = FALSE, include.dirs = FALSE))
		
		
		sw('..', showWarnings=F)
		
		rencopyDir= function(d, outd) { 
			if(!file.rename(d, fp(outd, d))){
				dir.create(fp(outd, d))
				for(f in dir(d))try({
								if(!file.rename(fp(d,f), fp(outd, d, f))){
									warning(sf("can't rename %s to %s", fp(d,f), fp(outd, d, f) ))
									if(!file.copy(fp(d,f), fp(outd, d, f)))warning(sf("can't  copy %s to %s", fp(d,f), fp(outd, d, f) ))
								} 
							})
		...
		try({	
					rencopyDir('out', outRelDir)
					rencopyDir('img', outRelDir)
					for(f in dir2('\\.', 'out'))file.copy(f, fp(outRelDir, f))
					
#					stopifnot(file.rename('out', fp(outRelDir, 'out')))
#					stopifnot(file.rename('img', fp(outRelDir, 'img')))
#					if(!file.rename('img', fp(outRelDir, 'img'))){
#						dir.create(fp(outRelDir, 'img'))
#						for(f in dir('img'))try({
#								if(!file.rename(fp('img',f), fp(outRelDir, 'img', f))) file.copy(fp('img',f), fp(outRelDir, 'img', f))
#							})
#						
#					}
#					
#					for(f in dir( include.dirs = F))file.copy(afp(f), gsub('\\.\\.', '.', outRelDir) %+% '/')
#					stopifnot(file.rename('out', gsub('\\.\\.', '.', outRelDir) %+% '/out'))
#					stopifnot(file.rename('img', gsub('\\.\\.', '.', outRelDir) %+% '/img'))
#			stopifnot(file.copy('*.*', gsub('\\.\\.', '.', outRelDir) %+% '/'))
		})
