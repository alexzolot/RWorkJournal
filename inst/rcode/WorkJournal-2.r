...
#' @title getQuoteCommentStatusA
#' @aliases getQuoteCommentStatusA 
getQuoteCommentStatusA= function(s,  verb=FALSE) {
	...
	for (i in seq(along = ss)) { s1= ss[i]
		...
		if(     s1== '"'  && state == 'o') {s2= '<span class="d">'  ; state='d'; s3= ''}  # use switch ? 
				...
				|| (s1== "'"  && state == 's')) {
			state='o'; 
			sOut[i]= s2 %+%  markdownToHTML(text= gsub('[ \t]{2, }',  '',  s3),  stylesheet= '',  header= '',  fragment.only=TRUE) %+% '</span>'
		}
		#|| (s1== "'"  && state == 's')) {state='o'; sOut[i]= s2 %+%  markdownToHTML(text= gsub('zzzz',  '',  s3),  stylesheet= '',  header= '',  fragment.only=TRUE) %+% '</span>'}
...
{ # header.jQuery,  footer  for cc()
	header.jQuery= c('<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="en"><head><link rel="shortcut icon" href="">
					...
					<script src="TRUE:/mathjax-MathJax-v2.2-8-g727332c/mathjax-MathJax-727332c/MathJax.js?config=TeX-AMS-MML_HTMLorMML"></script>
					<script src="TRUE:/work/UseR-2013/lib/jquery-1.10.1.min.js"></script>
					<!--  -->
...
#' @title ccm= code2HTML.md
#' @aliases ccm code2HTML.md 
ccm= code2HTML.md=  function(.theFile= get.theFile(),  img='img',  FullSyntaxHighlight= FALSE
	...
	s1= readLines(.theFile,  warn=FALSE)
	...
	s1='## Test1
			...
			* list4
			...
			'
	out= getQuoteCommentStatusA(s= s1,  verb=TRUE)
	# out
...
#' @title cc= code2HTML= code2HTMLjQuery
#' @aliases cc code2HTML code2HTMLjQuery 
cc= code2HTML= code2HTMLjQuery= function(.theFile= get.theFile(),  img='img',  FullSyntaxHighlight= FALSE
	...
	}else{s1= readLines(.theFile,  warn=FALSE)
	}  #--
...
#' @title CleanSparePics
#' @aliases CleanSparePics 
...
#' @title ReleaseOut
#' @aliases ReleaseOut 
ReleaseOut= function(vers='',  exec=FALSE) {
...
MakeRWJournals= function(root='TRUE:',  patt='.*',  pattNeg='^$',  exec=FALSE,  ...) {
...
if(0) RWJournals= MakeRWJournals(root='M:',  patt='71_UseR-2013-Tutorial.*59.zz',  pattNeg='zExtraPacks|999|scripts|library|lib|fun|Base|code2HTML|86_testShiny'
		,  exec=FALSE,  show=TRUE,  toSave=TRUE,  outSuffix='.b.htm')
...
#' @title gff
#' @aliases gff 
gff= function(patt=' ===',  pattNeg='gff',  f= get.theFile(),  withLineNumb= TRUE){ 
	catt(3099,'============================ gff:',  f)
...
#' @examples 
#'  \dontrun{
#'   CreateNewProj(newProj.name= '96_aaaa',  root='../../')
#' }
...
CreateProj= CreateProject= CreateNewProj= function(newProj.name= 'newProjTemplName'
		,  Templ.dir= system.file('newProjTemplName',  package ='WorkJournal')  # 'TRUE:/work/UseR-2013/99_commonR/newProjTemplName'
		,  root= gw()  # 'c:/'
...
#' @aliases 	listFuncUsage 
	...
	if(0){
		...
		fs= dir('M:/',  recursive=TRUE,  patt='\\.[rR]$')
...
#' @title subSingleQuote2div
#' @aliases subSingleQuote2div 
subSingleQuote2div= function(quote='^\\s*`\\s*$',  pattNeg='\'\\`\'|\"\\`\"',  repl=c('<div class="sq">',  '</div>'),  s1= readLines(theFile,  warn=FALSE)){
...
#' @title getQuoteCommentStatus
#' @aliases getQuoteCommentStatus 
getQuoteCommentStatus= function(s,  verb=FALSE) {
	sOut= ss= strsplit(pas(s,'',  '\n'),  '')[[1]]
...
#' @title ccc= parse.single.line.quotes
#' @aliases ccc parse.single.line.quotes 
ccc= parse.single.line.quotes= function(file= get.theFile(),  s=readLines(file),  verb=FALSE,  exec=TRUE) {
		...
		if (0) {
			...
			gw()
			htm= markdownToHTML(text= s3,  fragment.only=FALSE)  ; prr(htm)
			markdownToHTML(output='zz.htm',  text= s3,  fragment.only=FALSE)
			cat(htm,  file='zz.htm')
			expl('zz.htm')
			file.remove('zz.htm')		
		}
		...
		for(i in 1:le(s2)){ # i=3
			if(i%%2==1){#  R code
				#s4[[i]]= c('\n<pre><code>\n',  s2[[i]]) # r code
				s4[[i]]= markdownToHTML(text= c('',  '    ' %+% s2[[i]],  ''),   fragment.only=FALSE) # r code
			} else{ 
				minide=max(0,  min(u2[[i]]$ident))
				txt= substr(s2[[i]][-1],  minide+1,  999)
				ides= pas(rep(' ',  minide))
				htm= ides %+% markdownToHTML(text= txt,  fragment.only=TRUE)
				s4[[i]]= c(s2[[i]][1],  '\n</code></pre>\n\n',  htm) 
			}
		}
		
		#s4= markdownToHTML(text= s %+% '\n',  fragment.only=FALSE) #xxx !!!
		s5= c('<html>',  unlist(s4) %+%  '\n',  '</html>'); if( verb) prr(s5)
		if (exec) {
			cat(s5,  file='zz.htm')
			expl('zz.htm')
...
# u= parse.single.line.quotes(file= get.theFile(),  verb=FALSE,  exec=TRUE)
# gw()


#' @title ccc2= parse.terminal.line.quotes2
#' @aliases ccc2 parse.terminal.line.quotes2 
ccc2= parse.terminal.line.quotes2= function(file= get.theFile(),  s=readLines(file),  verb=FALSE,  exec=TRUE) {
	s=readLines('m:/50_HLP/out/HLP_demo/HLP_demo.r')
	
	#sing.quo= gregexpr('^\\s*([\'"`])\\s*$',  s)
	s= gsub('\t','    ',  s)  # prr(s)
	sing.quo= regexpr('^\\s*([\'"`])\\s*$',  s)
	q1= gre2('^\\s*[\'"`]',  '^\\s*([\'"`]).*\\1\\S',  s,  v=FALSE)  # line start quote
	q2= gre2('[\'"`]\\s*$',  '\\S([\'"`]).*\\1\\s*$',  s,  v=FALSE)  # line end quote
	ich= cumsum(q2 | q1)
	...
	u= df(s,  iich= ich - ifelse(ich%%2,  0,  c(0,  diff(ich))),  ich,  is.code=1- iich%%2,  text, q1, q2,  stringsAsFactors = FALSE)
	chunks= dlply(u,  .(iich,  is.code),  I)
	
	require(markdown)
#	sh= unlist(sapply(chunks,  function(cha){if(cha$is.code[1])c("\n<pre><code>",  cha$text,  "</code></pre>\n" ) 
#						else markdownToHTML(text=cha$text,  fragment.only=TRUE)}))
	...
	markdownToHTML('m:/62_MM_dispos/out/zz.md',  'zz.htm',  fragment.only=FALSE)
...
#' @title sg= HHp
#' @aliases sg HHp 
sg= HHp= function(capt=.main,  Width = dev.size(units = "px")[1] ,  off= TRUE,   Height =  dev.size(units = "px")[2]
		,  GraphPointSize = 12,  dirr='../img',  type= "cairo",  res=96,  dev=0,  fNameWithCapt=FALSE,  gg=FALSE,  ...){ # type= "windows"
	op= options(); on.exit(options(op))  #; options(error=dummy)
	if(!file.exists(dirr)) dir.create(dirr)
	.iPic<<- max(0,  nu(gsub('^Pic_(\\d+).*\\.png$',  '\\1',  dir(dirr,  patt='.png$'))),  na.rm=TRUE)
...
#		install.packages('C:/Users/azolotovitski/Downloads/rCharts-master.zip',  repos = NULL,  type='win.binary')
	...
	df(fs<- dir(root,  rec=TRUE, patt='^[^z].*R$'))
...
#' @title 	sgj= HHjp
#' @aliases 	sgj HHjp 
	sgj= HHjp= function(p= p1,  capt='',  dirr='../img',  absPath=TRUE) {
		.ijPic<<- max(0,  nu(gsub('^jPic_(\\d+).*\\.htm$','\\1',  dir(dirr,  patt='.htm$'))),  na.rm=TRUE)
...
# s= replaceTagsOutSq(readLines('M:/85_Otto/85_Otto.r',  warn=FALSE)); prr(tail(s))
#' @examples 
#'  s= replaceTagsOutSq(c(" <aa '<bb'   `<ee`  <cc",  " <aa '<bb'   `<ee`  <cc")); prr(s)
#' @title replaceTagsOutSq
#' @aliases replaceTagsOutSq 
replaceTagsOutSq= function(s= readLines('M:/85_Otto/85_Otto.r',  warn=FALSE)){
...
#' }
#' @title c2r= Comments2Roxygen
#' @aliases c2r Comments2Roxygen 
c2r= Comments2Roxygen= function(file= get.theFile(),  suffOut='-copy'
	...
	s= fsub(fin=file
			...
			,  HaLaP= 'HaLaP',  RWorkJournal= 'RWorkJournal')
	
	s= readLines(fout,  warn = FALSE)
...
if (0) {
	...
	sw('TRUE:/work/zPacks/HaLaP')
	# gw: sw("TRUE:/work/zPacks/HaLaP");  expl()
	
	theFile= 'TRUE:/work/zPacks/HaLaP/R/HaLaP.r'
	c2r(file=theFile,  suffOut='-copy',  exec=FALSE) 
	c2r(file=theFile,  suffOut='',  fsub=TRUE,  exec=TRUE)
	file.remove('TRUE:/work/zPacks/HaLaP/R/HaLaP-copy.r')
}
